...
El sistema propuesto consta de funciones de pertenencia \textit{triangulares} y \textit{trapezoidales}. También posee los métodos de agregación de \textbf{Mamdani} y \textbf{Larsen} al igual que todos los métodos de desdifusificación (\textit{Centroide}, \textit{Bisección} y todas las variantes de los \textit{Máximos}). Admite mútiples variables de entrada y para simular mútiples variables de salida, en la entrada se debe replicar la regla tantas veces como variables de salida haya.
...
Primeramente son \textit{parseadas} a través del método \lstinline|parse_rule| el cual devuelve una instancia de tipo \lstinline|Rule|. En una cláusula del tipo $Id \; 'is' \; Id $, el primer terminal \textit{Id} representa el nombre de la variable del sistema, mientras que el segundo sería el nombre del conjunto difuso correspondiente a la variable en cuestión. Lo anterior se cumple de la misma forma para las cláusulas negativas.
...
\textit{Mamdani} y \textit{Larsen} arrojaron resultados similares para una misma entrada, no siendo así si se utilizaban las funciones de desdifusificación \lstinline|left_max| o \lstinline|right_max|.
